...
function transformToLis(obj) {
  // Solution code here...
  return Object.entries(obj).map(key => `<li>${key[0]}: ${key[1]}</li>`);
}
...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((number, newArr) => {
    const totalSum = newArr.reduce((newTotal, vari) => vari === target ? newTotal + 1 : newTotal, 0);
...
const totalSum = (input) => {
  ...
  for (let array of input) {
    totalArr.push(array.reduce((num, total) => total + num, 0));
  } return totalArr.reduce((num, total) => total + num, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(newArr => newArr.filter(value => value % 5 === 0 && typeof value === 'number').map(filteredValue => Math.pow(2, filteredValue)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(starWars => starWars.gender === 'male' || starWars.gender === 'female').map(starWars => starWars.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  let shortest = data.reduce((tall, character) => {
    if (tall.height < character.height) {
      tall = character;
    }
    return tall;
  });
