...
const toTitleCase = (arr) => arr.map(string => string.replace(string[0], string[0].toUpperCase()));
// Solution code here...
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(large => +large.mass > +arr[0].mass).map(luke => luke.name).join(' - ');
};
